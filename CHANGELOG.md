...
- **15-Rule Validation Engine**: Automated multi-category rules (`VAL-001` to `VAL-015`) with severity scoring (`CRITICAL`, `HIGH`, `MEDIUM`, `LOW`).
- **Explainable AI Copilot Service**: Dual-engine architecture integrating Google Gemini API (`gemini-2.5-flash`) with a deterministic local financial heuristic engine for 100% offline resilience.
